...
Powered by Together AI ($5 free credits)
Integrates: Brave Search, Firecrawl, Apify, Together AI LLM
"""
...
class SPECTEROSINT:
    ...
    async def brave_search(self, query: str, count: int = 10) -> List[Dict]:
        """Search the web via Brave Search API (2000 queries/month free)"""
    ...
    # ═══════════════════════════════════════════════════════════════
    # FIRECRAWL — Website forensics
    # ═══════════════════════════════════════════════════════════════
    async def scrape_website(self, url: str) -> Dict[str, Any]:
        """Deep scrape a website for forensic analysis (500 pages/month free)"""
            ...
            if resp.status_code == 200:
                ...
                if data.get("success"):
                    return {
                        ...
                        "content": data.get("data", {}).get("markdown", "")[:8000],
                        "links": data.get("data", {}).get("links", [])[:50],
                        "source": "firecrawl"
                    }
    ...
    async def analyze_website_risk(self, url: str) -> Dict[str, Any]:
        """Scrape + AI analysis of a scam website"""
...
{content[:6000]}"""
        ...
        ai_result = await self._together_analyze(prompt)
        return {
            "url": url,
    ...
    async def scrape_twitter_profile(self, handle: str) -> Dict[str, Any]:
        """Scrape X/Twitter profile via Apify ($5 free credits)"""
        if not self.apify_key:
            return {"error": "APIFY_API_KEY not configured"}
        try:
            # Trigger Apify Twitter Profile Scraper
            ...
            if run_resp.status_code in (201, 200):
                ...
                return {
                    ...
                    "apify_run_id": run_id,
                    "status": "queued",
                    "check_url": f"https://api.apify.com/v2/actor-runs/{run_id}/dataset/items"
                }
    ...
    # ═══════════════════════════════════════════════════════════════
    # TOGETHER AI — Analysis engine
    # ═══════════════════════════════════════════════════════════════
    async def _together_analyze(self, prompt: str, model: str = "meta-llama/Llama-3.3-70B-Instruct-Turbo") -> Dict[str, Any]:
        """Send analysis prompt to Together AI"""
        ...
        try:
            ...
            return {"error": f"Together AI HTTP {resp.status_code}", "body": resp.text[:500]}
        except Exception as e:
            return {"error": str(e)}
    ...
    async def investigate_project(self, project_name: str, website: Optional[str] = None, twitter: Optional[str] = None) -> Dict[str, Any]:
        """Full OSINT investigation of a crypto project"""
        queries = [
            f"{project_name} crypto scam",
            f"{project_name} rug pull",
            f"{project_name} token review"
        ]
        if twitter:
        ...
        if website:
            website_analysis = await self.analyze_website_risk(website)
...
{json.dumps(evidence, indent=2)[:8000]}"""
        ...
        evidence["synthesis"] = await self._together_analyze(synthesis_prompt)
        return evidence
    ...
    def _guess_domain_age(self, url: str) -> str:
        """Heuristic guess based on domain TLD and patterns"""
        parsed = urlparse(url)
        domain = parsed.netloc.lower()
        if any(x in domain for x in [".xyz", ".top", ".click", ".link", ".icu"]):
            return "likely_new"
        if any(x in domain for x in [".com", ".io", ".co"]):
            return "unknown"
        return "unknown"


# Singleton instance
_specter: Optional[SPECTEROSINT] = None
