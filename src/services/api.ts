...
class ApiClient {
  ...
  // ═══════════ UNIFIED CRYPTO INTELLIGENCE (15+ sources) ═══════════
  async cryptoUnifiedPrice(tokenAddress: string, chain: string = 'solana', symbol: string = ''): Promise<any> {
    const response = await this.client.get(`/api/v1/crypto/unified/price/${tokenAddress}?chain=${chain}&symbol=${symbol}`);
    return response.data;
  }

  async cryptoUnifiedData(tokenAddress: string, chain: string = 'solana'): Promise<any> {
    const response = await this.client.get(`/api/v1/crypto/unified/data/${tokenAddress}?chain=${chain}`);
    return response.data;
  }

  async cryptoUnifiedTrending(chain: string = 'solana'): Promise<any> {
    const response = await this.client.get(`/api/v1/crypto/unified/trending?chain=${chain}`);
    return response.data;
  }

  async cryptoUnifiedWallet(address: string, chain: string = 'solana'): Promise<any> {
    const response = await this.client.get(`/api/v1/crypto/unified/wallet/${address}?chain=${chain}`);
    return response.data;
  }

  async cryptoKeysStatus(): Promise<any> {
    const response = await this.client.get('/api/v1/crypto/keys');
    return response.data;
  }
  ...
  async getProviderBudget(): Promise<any> {
    const response = await this.client.get('/api/v1/budget/summary');
    return response.data;
  }
}
